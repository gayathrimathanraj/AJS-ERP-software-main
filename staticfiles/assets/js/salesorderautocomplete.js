console.log('salesorderautocomplete.js loaded');

// Move this function OUTSIDE document ready
function calculateSalesOrderWithBackend() {
    console.log('🔄 Calculating Sales Order with Django backend...');
    ...
    $.ajax({
        url: '/ajserp/create_sales_order/',  // CHANGED URL for sales order
        method: 'POST',
        ...
        success: function(response) {
            if (response.success) {
                ...
                alert('✅ Sales Order Calculations completed successfully!');
            } else {
                alert('❌ Error: ' + response.error);
            }
        },
        error: function(xhr, status, error) {
            alert('❌ Sales Order Calculation failed: ' + error);
        },
        complete: function() {
            saveBtn.prop('disabled', false).text('Save');
        }
...
$(document).ready(function(){
    // Page detection - only run on sales order pages
    const currentPath = window.location.pathname;
    if (!currentPath.includes('salesorder') && !currentPath.includes('addsalesorders')) {
        return;
    }
    ...
    $('#salesOrderForm').on('submit', function(e) {  // CHANGED FORM ID
        if (!validateSalesOrderForm()) {  // CHANGED FUNCTION NAME
            e.preventDefault();
            return false;
        }
        return true;
    ...
    function handleRowAutocomplete(inputElement, suggestionsDiv) {
        ...
        typingTimer = setTimeout(function(){
            ...
            $.ajax({
                url: "/ajserp/api/materialestimate-autocomplete/",  // Same API as estimate
                data: {q: query},
                dataType: "json",
    ...
    function handleCustomerSearch(inputElement) {
        ...
        typingTimer = setTimeout(function(){
            $.ajax({
                url: "/ajserp/api/customer-autocomplete/",  // Same API as estimate
                data: {q: query},
                dataType: "json",
                success: function(data){
                    let items = "";
    ...
    function handleWarehouseSearch(inputElement) {
        ...
        typingTimer = setTimeout(function(){
            $.ajax({
                url: "/ajserp/api/warehouse-autocomplete/",  // Same API as estimate
                data: {q: query},
                dataType: "json",
                success: function(data){
                    let items = "";
    ...
    function selectMaterial(material) {
        ...
        if (existingMaterial.length > 0) {
            alert('This material is already added to the sales order');
            return;
        }
    ...
    function initializeDateFields() {
        ...
        $('input[name="delivery_date"]').val(deliveryDate.toISOString().split('T')[0]);
    }
    ...
    function populateMaterialRow(row, material) {
        ...
        getTaxRates(material.hsn_code).then(taxRates => {
            ...
            calculateSalesOrderWithBackend();
    ...
    function getTaxRates(hsnCode) {
        return new Promise((resolve, reject) => {
            ...
            $.ajax({
                url: "/ajserp/api/get-tax-rates/",  // Same API as estimate
    ...
    // Form validation
    function validateSalesOrderForm() {  // CHANGED FUNCTION NAME
    ...
    function addEmptyRow() {
        const tBody = $("#sales-order-items-body");  // CHANGED ID
    ...
    window.calculateSalesOrderWithBackend = calculateSalesOrderWithBackend;
});